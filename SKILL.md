---
name: tech-ppt-skill
description: 生成「技术风、高信息密度」的单文件 HTML deck —— 全程等宽字体（Ubuntu Mono）、零动效、可快速连续翻页、每页带 #锚点、页数严格 ≤16 页、一页讲清一个模块/子问题。排版以表格 / 定义列表 / 指标 / 代码块为主，设计感克制，单页信息密度明显高于 guizang-ppt-skill / frontend-slides 那类"大字 + 留白"的演示。当用户要做技术分享、架构评审、协议/系统拆解、工程汇报，且强调"信息密度大""技术风""等宽""一页讲清一个模块""不要花哨动效"时使用；需要解释结构/流程/时序时，可用 mermaid 预渲染为内联流程图/架构图（静态 SVG，自动适配主题）。
---
...
- **页数 ≤ 16**（硬约束）：封面 1 + 中间 ≤14 + 收尾 1。一页一个模块/子问题。
- **背景统一**：整份 deck 要么全浅、要么全深（见 `references/themes.md` 选 light 或「暗终端」dark），**不得逐页深浅混用**。
...
- **图解（按需）**：需要讲清结构/流程/时序时，用 mermaid 经 `scripts/mermaid-svg.mjs` **预渲染为静态内联 SVG**（不在浏览器跑运行时），仍单文件/离线/零动效；图自动适配主题与暗页。详见 `references/diagrams.md`。
...
### Step 3 · 填充 ≤16 页
打开 `references/layouts.md` 挑版式骨架（L1–L12）并**逐页轮换**，粘贴改文案。每页要点：
1. 一页只讲**一个**模块/子问题；
2. 但这一页要**塞满**——并排放定义/表格/指标/代码，密度高于普通 deck；
3. **版式轮换防雷同**：先按内容性质选骨架（流程→L8 / 步骤·原则→L9 / 并列→L10 / 过渡→L11 / 代码·大图→L12 / 对照→L3·L5 / 数据→L3·L4），相邻页不用同一骨架，L2 最多连用 2 页，≥6 页的 deck 至少 4 种版式；
4. 每页给唯一 `id`，并在底部 `.pager` 同步加一条 `<a href="#id">NN</a>`；
5. 控制总页数 ≤ 16。

### Step 3.5 · （按需）画流程图 / 架构图
仅当「结构 / 流程 / 分支 / 时序」用表格讲不清时才画——一份 deck 通常 0–2 张。用 mermaid 写，经
`<SKILL_ROOT>/scripts/mermaid-svg.mjs flow.mmd -o flow.svg` **预渲染为静态内联 SVG**（自动随主题/暗页着色），
把 `<svg>…</svg>` 粘进 `.diagram` 容器。何时/如何画见 `references/diagrams.md`，版式见 `layouts.md` L7。图也要过 Step 4 溢出检测。
...
tech-ppt-skill/
├── SKILL.md                    ← 本文件
├── package.json                ← 声明 beautiful-mermaid 依赖（画图用；node_modules 已 gitignore）
├── assets/template.html        ← 种子模板（等宽 / 零动效 / 锚点导航 / 图解容器，完整可运行）
├── references/
│   ├── themes.md               ← 4 套视觉基调（只换配色，不换字体/排版）
│   ├── layouts.md              ← 高密度版式骨架（L1–L7，可粘贴）
│   ├── diagrams.md             ← 何时/如何画图（mermaid → 预渲染内联 SVG）
│   └── checklist.md            ← 交付前自检清单
└── scripts/
    ├── check-overflow.sh       ← headless 溢出检测（质量门）
    └── mermaid-svg.mjs         ← mermaid → 干净内联 SVG（beautiful-mermaid，自动适配主题/暗页）
```
