...
1. **一页一模块** —— 标题回答"这页讲哪个子问题"，主体把它讲透。
2. **密度即价值** —— 空白多 = 没讲够。优先表格/定义列表组织密集事实。
3. **等宽到底** —— 等宽是这套 deck 的身份，数字对齐、代码原生、工程感来源。
4. **零动效、可连翻** —— 体验上"啪啪啪"翻得动，是和其它两个 skill 的关键区别。
5. **检测优先于感觉** —— 交付前 `check-overflow.sh` 必须全绿。
